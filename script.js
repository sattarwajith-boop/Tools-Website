...
async function initAnimations() {
  ...
  try {
    const { animate, inView, stagger } = await import("./node_modules/motion/dist/es/index.mjs");
...
function initSearchAndFavorites() {
  ...
  const refreshToolDiscovery = () => {
    ...
    if (countLabel) {
      countLabel.textContent = `${visibleCount} tool${visibleCount === 1 ? "" : "s"} visible`;
    }
    if (emptyState) {
  ...
  const renderFavoritesShelf = () => {
    ...
    const favoriteCards = toolCards.filter((card) => favorites.has(card.querySelector(".favorite-toggle")?.dataset.favorite || ""));
    ...
    favoritesList.innerHTML = favoriteCards
      .map((card) => {
        ...
        const anchor = card.querySelector(".text-link")?.getAttribute("href") || "#tools";
        const category = card.dataset.toolCategory || "general";
        return `
          <a class="favorite-link-card" href="${anchor}">
  ...
  favoriteButtons.forEach((button) => {
    ...
    button.addEventListener("click", () => {
      ...
      announce(`${button.closest(".tool-card")?.dataset.toolTitle || "Tool"} ${favorites.has(key) ? "saved to favorites" : "removed from favorites"}.`);
    });
  });
