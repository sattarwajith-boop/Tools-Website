...
function initAnimations() {
  ...
  scroll(() => {
    ...
    if (topbar) {
      topbar.style.transform = `translateY(${Math.min(offset * -0.03, -4)}px) scale(${1 - Math.min(offset * 0.00025, 0.025)})`;
      topbar.style.boxShadow = `0 ${24 + Math.min(offset * 0.08, 10)}px ${70 + Math.min(offset * 0.12, 18)}px rgba(17, 28, 44, 0.08)`;
    }
    if (pageNoise) {
